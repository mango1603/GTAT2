...
var vx0Ball = -3.8; 							// Startgeschwindigkeit 300 km/h = 300/3,6 m/s
var vy0Ball = 0;
...
/* administration */
	push();
		...
		textAlign(CENTER);
		text("(5.) Das ultimative Golf-Spiel", 50*gridX, 10*gridY);
		textSize(fontSize);
					...
					switch (status)															// Berechnung ausführen
						{
							...
							case "3.slope": 	vs = vs + g_*dt;							// Wegberechnung für Rollbewegung
												...
												break;
							case "on flight":	vyBall = vyBall + g*dt;						// Wegberechnung Flug
