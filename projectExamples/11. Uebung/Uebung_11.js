...
var myR_Rasen = 0.2,
    myR_Sand = 0.3; // Rollreibungskoeffizienten				
...
var dBall = 0.048; // Balldurchmesser optisch
...
var vy0Ball = 0;
var dMin, iMin = 0; // minimale Distanz Ball - Playground, Segmentnummer
...
function setup() { /* prepare program */
    ...
    createUnitVector(); // Einheitsvektor der Playground-Segmente
}
...
function draw() {
    ...
    if (debug) {
        ...
        text("vxBall: " + nf(vxBall, 2, 3) + "    vyBall: " + nf(vyBall, 2, 3), 14 * gridX, 26 * gridY);
        text("\u03b2: " + nf(degrees(beta), 3, 0), 14 * gridX, 28 * gridY);
    }
    ...
    if (START) {
        ...
        if (next || !debug) {
            ...
            if (status == "start" || PutterInMove) {
                ...
                if (PutterInMove) damping = dampPutter;
                else damping = 0;
            ...
            switch (status) // Berechnung vorbereiten
            {
                ...
                case "1.plane":
                    if (len_i[0] > s && xBall <= P[0][0]) { // Ortsberechnung
                ...
                case "1.slope":
                    if (len_i[1] > s && s >= 0) { // Zeitfunktion berechnen
                        g_ = g_i[1];
                        beta = beta_i[1];
                        len = len_i[1];
                        aRR = aRR_i[1]; // wirksame Reibungsverzögerung
                        ...
                        if (s >= len_i[1]) { // Übergang 1.slope -> flight
                            status = "on flight";
                        ...
                        if (s <= 0) { // Übergang 1.slope -> 1.plane
                            status = "1.plane";
                ...
                case "2.slope":
                    if (len_i[2] > s && s >= 0) {
                        ...
                        if (s >= len_i[2]) { // Übergang 2.slope -> 2.plane
                            status = "2.plane";
                ...
                case "on flight":
                    ...
                    for (var i = 0; i < P.length - 1; i++) {
                        var result = getDistance(xBall, yBall, i);
                        if (result[1] <= dMin && result[2] < len_i[i] && result[2] > 0) // Distanz Ball - Playground Punkt
                        {
                            iMin = i; // Segmentnr. kleinste Distanz
                            dMin = result[1]; // kleinste Distanz
                            s = result[2];
                        }
                        //console.log(i+" "+nf(result[0],2,4)+" d: "+nf(result[1],2,4)+" p: "+nf(result[2],2,4));													
                    }
                    if (dMin <= 0.5 * dBall) {
                        console.log(iMin + " d:" + nf(dMin, 2, 4) + " status: " + segmentToStatus[iMin] + "  s: " + nf(s, 2, 4) + "m von " + nf(len_i[iMin], 2, 4) + "m");
            ...
            switch (status) // Berechnung ausführen
            {
                ...
                case "4.plane":
                case "3.slope":
                    vs = vs + (g_ - sign * aRR) * dt; // Wegberechnung für Rollbewegung mit Rollreibung
                    if (abs(vs) < 0.03 && beta == PI) // Ende der Bewegung -> Energie aufgebraucht
                        dt = 0;
                    else {
                        if (vs > 0) sign = 1;
                        else sign = -1; // Reibkraft ist der Bewegung entgegen gerichtet!
                        s = s + vs * dt;
                    }
    ...
    switch (status) {
        ...
        case "end":
            ...
            break;
        case "on flight":
            ballOnFlight(xBall, yBall);
            if (debug) {
                stroke('#ff0000');
                line(P[iMin][0] * M, P[iMin][1] * M, xBall * M, yBall * M); // Objektvektor
            }
            break;
        case "water":
            push();
