...
function PushButton(xPos, yPos, onName, onColor, modus) { // Anklicken mit der Maus erzeugt einen einmaligen Impuls, Button muss enabled werden!
    this.xPos = xPos;
    this.yPos = yPos;
    this.onName = onName;
    ...
    this.drawButton = function(enable) // inactive: gray
        {
            ...
            // Buttongestaltung
            if (!enable) fillColor = '#aaaaaa';
            else fillColor = OnColor;
            stroke(textColor);
            fill(fillColor);
...
function Handle(M, D, R, dR, visible, c) {
    ...
    this.moveHandle = function(xPos, yPos, x, y) { // Eingabe von Vektoren, z.B. Geschwindigkeiten etc. 
    ...
    this.inCircle = function(xPos, yPos) // Pos in Metern und im kart. Koord.Syst.
        {
            ...
            /* Maus in der Ellipse? */
            if (sq(iXk(mouseX) - xPos * Scale) + sq(iYk(mouseY) - yPos * Scale) <= sq(R * Scale)) { // im Kreis, sensibler Kreis ist 1.5 mal größer als der sichtbare Kreis -> für kleine Mobilgeräte
            ...
            } else { // außerhalb des Kreises -> Koordinaten bleiben unverändert
            ...
            if (overBubble == true) fill(manageColor(this.c)[0]);
            else fill(this.c)
            if (this.visible) {
...
function kXi(a) { /* a ist beliebige kart. Größe */
    return (a + xi0);
}

function kYi(b) { /* b ist bel. kart. Größe */
    return (yi0 - b);
}

/* Transformation interner in kartesische Koordinaten */
function iXk(a) { /* a ist beliebige interne Größe */
    return (a - xi0);
}

function iYk(b) { /* b ist bel. interne Größe */
    return (yi0 - b);
}
...
function mouseClicked() { /* der Merker mClick muss nach Gebrauch extern rückgesetzt werden! */
    mClicked = true;
    //alert("mouse");
    //isTouchscreen = false;
}
...
function mouseReleased() { /* der Merker mReleased muss nach Gebrauch extern rückgesetzt werden! */
    mReleased = true;
    mPressed = false;
    mClicked = false;
...
function mouseDragged() { /* der Merker mDragged muss nach Gebrauch extern rückgesetzt werden! */
    mDragged = true;
}

function manageColor(c) { // input hex color, returns inverted color, brighness, dec. RGB-values
